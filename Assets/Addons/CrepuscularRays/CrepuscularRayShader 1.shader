...
Shader "Lighting/Crepuscular Rays" {
	...
	Properties{
		...
		_fogInfluence("Fog Influence", Range(0,10)) = 0.5
		...
		uniform sampler2D_half _spotRT;
		...
		half rand(half2 co)
				{
				    ...
				    return sin(frac(sin(sn) * c)* ((_Time.w * _fogSpeed) * _fogInfluence));
				}	
		...
		half4 frag(v2f i) : COLOR
			{
				...
				half4 spotlight = half4(_spotPos.xyz,1);
				...
				//get x and y direction, because we need to manipulate the uv's manually to get the 'shadows' to
				//track the spotlight's constrained rotation somewhat correctly 
				half2 spotDeltaTexCoord;
				spotDeltaTexCoord.y = spotlight.y > 1.5h ? i.uv.y + spotlight.y + 0.5  : i.uv.y - spotlight.y + 0.25h;
				spotDeltaTexCoord.x = spotlight.x > 1.0h ? i.uv.x + spotlight.x : i.uv.x + spotlight.x - 1;
				
				// Divide by number of samples and scale by control factor.
				deltaTexCoord *= 1.0h / _NumSamples * _SpotDensity;
				spotDeltaTexCoord *= 1.0h / _NumSamples * _SpotDensity;
				// Store initial sample.
				half2 uv = i.uv;
				half2 uv2 = i.uv;
				half4 color = tex2D(_MainTex, uv);
				half4 spotColor = tex2D(_spotRT, uv);
				...
				for (int i = 1; i < _NumSamples + 1; i++)
				{
					...
					half randomFactor = rand(uv.xy)*_fogInfluence * _Contrast;
					...
					//spotlight
					// Step sample location along ray.
					uv2 -= spotDeltaTexCoord;
					// Retrieve sample at new location.
					float sample2 = tex2D(_spotRT, uv2);
					float cast2 = Linear01Depth(tex2D(_CameraDepthTexture, uv2)).r;
					//calc depth value
					depth = float(value !=0 ? float(cast2 * 1.5f):float(cast2 * (1-randomFactor)));
					// Apply sample attenuation scale/decay factors.
					sample2 *= illuminationDecay * (_Weight/ _NumSamples*4) * depth;
					sample2 *= 3.h;
					
					// Accumulate combined color.
					spotColor += sample2;
					spotColor.a = 1-spotColor.a;
					// Update exponential decay factor.
					illuminationDecay *= _Decay;
				}
				
				// Output final color with a further scale control factor.
				return (max(half4(min(spotColor, color) * _Exposure), 0.05h));
			}
/////////////// SGX Horizontal Blur /////////////////////////////		
