...
public class Crepuscular : MonoBehaviour
{
	...
	{
		var blurTex = RenderTexture.GetTemporary(512, 256, 0, source.format);
	...
	if (blurSize > 0){
		for(int i = 0; i < 1; i++) {
                ...
                // vertical blur
                RenderTexture rt2 = RenderTexture.GetTemporary(512, 256, 0, source.format);
                ...
                // horizontal blur
                rt2 = RenderTexture.GetTemporary(512, 256, 0, source.format);
                rt2.filterMode = FilterMode.Bilinear;
