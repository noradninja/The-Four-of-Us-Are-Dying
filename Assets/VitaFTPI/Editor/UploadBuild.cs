...
public class UploadBuild 
{
	...
	static string DefaultInstallPath = "C:\\VitaFTPI-Core\\Tools\\Uploader";
	...
	static int loadData()
	{
		...
		{
			if (!data.CustomUploaderFolder) UploaderPath = DefaultInstallPath;
	...
	static int PreSetup()
    {
        ...
        {
			UnityEngine.Debug.Log("Please build via File > Build Settins > Build at least once before using this method!");
			return -1;
        }
	...
	public static string GetExtraArgs()
    {
        ...
        {
			toret += " --replace-install --compare";
			return toret;
        }

		toret += " --replace-install --partial";
		return toret;
    }
	...
	public static void UploadVPK()
	{
		...
		string args = "--vpk \"" + UploaderPath + "\\" + GetProjectName() + ".vpk\" --ip " + data.IP + " --usb " + boolToString(data.UseUSB) + " --storage-type " + data.storageType + " --upload-dir \"" + UploaderPath + "\"";
