...
Shader "Vita/Standard Mobile Foliage Wind"
{
    Properties
    {
...
#pragma multi_compile _ LOD_FADE_CROSSFADE
        ...
        Pass{
			...
			v2f vert( appdata v )
			{
				...
				( (v.vertex.x += cos(21 * v.vertex.x * 1 + (worldPos.x/1) ) * 0.15 * -1 * 5), //x
				(v.vertex.y += sin(15 * v.vertex.y * 1 + (worldPos.y/1) ) * 0.15 * 1 * 5),   //y
				(v.vertex.z += sin(cos(21 * v.vertex.z * 1 + (worldPos.z/1) ) * 0.15 * -1 * 5)) ); //z
