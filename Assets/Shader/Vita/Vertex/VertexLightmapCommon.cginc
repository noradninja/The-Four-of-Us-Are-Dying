...
v2f vert(appdata v)
{
    ...
    if (_LeavesOn)
    {
        ...
        ((v.pos.x += moveColor * sin(_Time.z * v.pos.x * _leaves_wiggle_speed + (worldPos.x / _wind_size)) *
                _leaves_wiggle_disp * _wind_dir.x), //x
            (v.pos.y += moveColor * sin(_Time.w * v.pos.y * _leaves_wiggle_speed + (worldPos.y / _wind_size)) *
                _leaves_wiggle_disp * _wind_dir.y), //y
            (v.pos.z += moveColor * sin(_Time.y * v.pos.z * _leaves_wiggle_speed + (worldPos.z / _wind_size)) *
