...
void TangentSpaceLightingInput(half3 normalWorld, half4 vTangent, half3 lightDirWorld, half3 eyeVecWorld,
                               out half3 tangentSpaceLightDir, out half3 tangentSpaceEyeVec)
{
...
VertexOutputBaseSimple vertForwardBaseSimple(VertexInput_VC v)
{
    ...
    if (_LeavesOn)
    {
        ...
        ((v.vertex.x += cos(_Time.z * v.vertex.x * _leaves_wiggle_speed + (posWorld.x / _wind_size)) *
                _leaves_wiggle_disp * _wind_dir.x * _influence), //x
            (v.vertex.y += sin(_Time.w * v.vertex.y * _leaves_wiggle_speed + (posWorld.y / _wind_size)) *
                _leaves_wiggle_disp * _wind_dir.y * _influence), //y
            (v.vertex.z += sin(
                cos(_Time.y * v.vertex.z * _leaves_wiggle_speed + (posWorld.z / _wind_size)) * _leaves_wiggle_disp *
                _wind_dir.z * _influence))); //z
    }
    ...
    TangentSpaceLightingInput(normalWorld, v.tangent, _WorldSpaceLightPos0.xyz, eyeVec, o.tangentSpaceLightDir,
                              tangentSpaceEyeVec);
    #if SPECULAR_HIGHLIGHTS
...
half4 fragForwardBaseSimpleInternal(VertexOutputBaseSimple i)
{
    ...
    half4 c = UNITY_BRDF_PBS(s.diffColor, s.specColor, rl, s.smoothness, s.normalWorld, -s.eyeVec, gi.light,
                             gi.indirect) * (attenuatedLightColor, 1);
...
half4 fragForwardAddSimple(VertexOutputForwardAddSimple_VC i) : SV_Target
// backward compatibility (this used to be the fragment entry function)
{
