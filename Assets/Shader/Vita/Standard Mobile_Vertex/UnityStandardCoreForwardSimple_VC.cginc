...
VertexOutputBaseSimple vertForwardBaseSimple(VertexInput_VC v)
{
    ...
    if (_LeavesOn)
    {
        ...
        ((v.vertex.x += sin(_Time.y * v.vertex.x * _leaves_wiggle_speed + (posWorld.x / _wind_size)) *
                _leaves_wiggle_disp * _wind_dir.x * (_influence * v.color)), //x
            (v.vertex.y += sin(_Time.y * v.vertex.y * _leaves_wiggle_speed + (posWorld.y / _wind_size)) *
                _leaves_wiggle_disp * _wind_dir.y * (_influence * v.color)), //y
            (v.vertex.z += sin(_Time.y * v.vertex.z * _leaves_wiggle_speed + (posWorld.z / _wind_size)) *
