...
// This assumes that the maximum param is right if both are supplied (range and map)
    float roughness = saturate(max(_Roughness + EPS, tex2D(_MetalnessMap, uv)).a);
    float metalness = saturate(max(_Metalness + EPS, tex2D(_MetalnessMap, uv)).r);
    float occlusion = saturate(tex2D(_MetalnessMap, uv)).g;
...
// INDIRECT LIGHTING
    float3 diffuseIrradiance = UNITY_SAMPLE_TEXCUBE_LOD(unity_SpecCube0, normal, UNITY_SPECCUBE_LOD_STEPS).rgb * occlusion;
    float3 specularIrradiance = UNITY_SAMPLE_TEXCUBE_LOD(unity_SpecCube0, reflectVec, roughness * UNITY_SPECCUBE_LOD_STEPS).rgb * occlusion;
...
// TODO: implement the split-sum approximation (UE4 paper)
    float3 ambientSpecular = specularIrradiance;

    return float4((brdfDirectOutput + ambientDiffuse + ambientSpecular) * lm, 1.0);
}
ENDCG
}
}
