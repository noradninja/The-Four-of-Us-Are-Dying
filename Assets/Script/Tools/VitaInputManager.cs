...
public class VitaInputManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Application.platform == RuntimePlatform.PSP2) // PSV/PSTV only 
        {
            ...
            {
                var dataMatrix = new float[3, 4]; // 3 rows: Finger ID, X, Y
                bool foundTL2 = false,
                    foundTL3 = false,
                    foundTR2 = false,
                    foundTR3 = false; // Track active touch presence
                int TL2ID = 255, TL3ID = 255, TR2ID = 255, TR3ID = 255; //initialize with OOB values
                for (var t = 0; t < currentBackTouchCount && t < 4; t++) // Limit to 4 touches
                {
                    var currentTouch = PSVitaInput.GetSecondaryTouch(t);
                    ...
                    var x = currentTouch.position.x;
                    var y = currentTouch.position.y;
    ...
    public static VitaInputManager Instance
    {
        get
        {
            if (!instance)
            {
                instance = FindObjectOfType(typeof(VitaInputManager)) as VitaInputManager;
                if (!instance) VitaDebug.Log("No active VitaInputManager script on any GameObject");
            }
