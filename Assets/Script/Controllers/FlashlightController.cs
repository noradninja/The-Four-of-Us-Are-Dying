...
public class FlashlightController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (lightFocusing && currentTarget != null && !FlashlightDisabled)
            if (currentTarget.GetComponentInParent<EnemyController>().isPlayerNear == true)
            {
                ...
                var lightdir = position - lightRoot.transform.position;
                var lightlookRotation = Quaternion.LookRotation(lightdir);
                var lightrotation = Quaternion.Lerp(lightRoot.transform.rotation,
                    lightlookRotation,
    ...
    private void LTrigEvent()
    {
                          ...
                          && !FlashlightDisabled && !FlashlightOff)
        {
            ...
            if (!PlayerController.isStimulant)
                LerpFocalLength(0.087f, 0.095f,
                    0.5f, 0.5f, 0.5f);

            if (UICanvasGroup.alpha < 1.0f) StartCoroutine(FadeAlpha(UICanvasGroup.alpha, 1.0f, 0.5f, 0.0f));
        }
    }
    ...
    private void SquareDownEvent()
    {
        ...
        {
            lightRoutine =
                FadeLightStaticInput(lightBeam.material.color, colorStart, 0.25f, flashlight.intensity, 5, 40, 40,
                    0.08f, 0.08f);
            StartCoroutine(lightRoutine);
    ...
    private IEnumerator LerpFocalLength(float StartValue, float endValue, float endBloom, float endThreshold,
        float duration)
    {
        var time = 0.0f;
        while (time < duration)
        ...
        float StartSize, float endSize)
    {
        ...
        while (time <= duration)
        {
            lightBeam.material.color =
                Color.Lerp(StartColor, endColor, time / (duration / 2)); //lerp the colors from dark to light
            lightHaze.material.color =
                Color.Lerp(StartColor, endColor, time / (duration / 2)); //lerp the colors from dark to light
        ...
        float StartSize, float endSize)
    {
        ...
        while (time <= duration)
        {
            lightBeam.material.color = Color.Lerp(StartColor, new Color
                    (endColor.r, endColor.g, endColor.b, endColor.a * currentCharge),
                time / (duration / 50)); //lerp the colors from dark to light
