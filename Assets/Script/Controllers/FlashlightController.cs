	...
	private void LTrigUpEvent()
	{
		if (HasFlashlight)
		{
			...
			if (!FlashlightDisabled && currentCharge >= 0.05f){
				...
				lightRoutine = FadeLightStaticInput(currentColor, colorStart, 0.25f, currentIntensity, 10.0f,
	...
	private void LTrigDownEvent()
	{
		...
		else if (HasFlashlight && !FlashlightDisabled && currentCharge >= 0.05f)
		{
			...
			lightRoutine = FadeLightDynamicInput(colorStart, colorEnd, duration, 
				currentIntensity, 50, 40, 25, 0.08f, 0.040f); // 'fire' light
			StartCoroutine(lightRoutine);
	...
	private void SquareDownEvent()
	{
		...
		if (FlashlightDisabled)
		{
			...
			FadeLightStaticInput(lightBeam.material.color, colorStart, 
					0.25f, flashlight.intensity, 10.0f, 40, 40, 
					0.08f, 0.08f);
		...
		{
			lightRoutine =
				FadeLightStaticInput(lightBeam.material.color, colorStart, 0.25f, flashlight.intensity, 10.0f, 40, 40, 0.08f, 0.08f);
			StartCoroutine(lightRoutine);
